...
public class SqLiteDatabaseHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase sqLiteDatabase) {
		 ...
		 */
		String sql_songs = "create table songs(songName varchar(20) ,singer varchar(20),songUri varchar(30),songImage varchar(100),singlyrics text,information varchar(50) ,primary key(songName,singer))";
		sqLiteDatabase.execSQL(sql_songs);
		/**
		 * 创建collection表  歌名 账号（参照用户的账号）
		 */
		String sql_collection= "create table collection(SongName varchar(20) primary key,account varchar(20) references user(account))";
		sqLiteDatabase.execSQL(sql_collection);
		/**
		 * 创建歌单表 歌单名  歌名 账号（参照用户的账号）
		 */
		String sql_songList= "create table songList(songListName varchar(20) primary key,SongName varchar(20) ,singer varchar(20),account varchar(20) references user(account))";
		sqLiteDatabase.execSQL(sql_songList);
		/**
		 * 创建专辑    专辑名 歌名 账号（参照用户的的账号）
		 */
		String sql_album = "create table album(albumName varchar(20) primary key,songName varchar(20) ,singer varchar(20),account varchar(20) references user(account))";
		sqLiteDatabase.execSQL(sql_album);
		/**
		 * 创建播放列表 歌名（参照音乐的歌名)播放列表可以重复歌名
		 */
		String sql_playList = "create table playList(songName varchar(20) references songs(songName))";
		sqLiteDatabase.execSQL(sql_playList);
		/**
		 * 创建最近播放列表  歌名 歌手
		 */
		String sql_recentlyPlayed = "create table recent(songName varchar(20),singer varchar(20) ,primary key(songName,singer))";
